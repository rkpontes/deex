...
mixin StateMixin<T> on ListNotifier {
  ...
  void _fillInitialStatus() {
    _status = (_value == null || _value!._isEmpty())
        ? RxStatus<T>.loading()
        : RxStatus<T>.success(_value as T);
  }
  ...
  void futurize(Future<T> Function() body,
    ...
    compute().then((newValue) {
      if ((newValue == null || newValue._isEmpty()) && useEmpty) {
        status = RxStatus<T>.loading();
      } else {
        status = RxStatus<T>.success(newValue);
      }

      refresh();
    }, onError: (err) {
      status = RxStatus.error(errorMessage ?? err.toString());
      refresh();
    });
  }
}
...
class DeexListenable<T> extends ListNotifierSingle implements DeexInterface<T> {
  DeexListenable(T val) : _value = val;
...
/// Deex Lifecycle
abstract class DeexNotifier<T> extends Value<T> with DeexLifeCycleMixin {
...
abstract class RxStatus<T> with Equality {
  const RxStatus();

  factory RxStatus.loading() => LoadingStatus<T>();

  factory RxStatus.error(String message) => ErrorStatus<T, String>(message);

  factory RxStatus.empty() => EmptyStatus<T>();

  factory RxStatus.success(T data) => SuccessStatus<T>(data);

  factory RxStatus.custom() => CustomStatus<T>();
}

class CustomStatus<T> extends RxStatus<T> {
  @override
  List get props => [];
}

class LoadingStatus<T> extends RxStatus<T> {
  @override
  List get props => [];
}

class SuccessStatus<T> extends RxStatus<T> {
  final T data;
...
class ErrorStatus<T, S> extends RxStatus<T> {
  final S? error;
...
class EmptyStatus<T> extends RxStatus<T> {
  @override
  List get props => [];
}

extension StatusDataExt<T> on RxStatus<T> {
